    ...
    <div class="content">
      <a-table :columns="scoreColumns" :data-source="scoreData" :pagination="false" :loading="scoreLoading">
        <template slot="score" slot-scope="text, record, index">
          <span v-if="index === 0">本期全网总得分：{{ record.score }}</span>
          <a-button type="link" v-else @click="handleClickStore(record.storeId)">{{ record.storeName }}大区得分：{{ record.score }} </a-button>
        </template>
